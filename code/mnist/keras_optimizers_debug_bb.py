...
class BB(Optimizer):
    ...
    def get_updates(self, params, constraints, loss):
        ...
        LR1 = theano.shared(1.)
        WUPDATE = [theano.shared(value=np.random.normal(size=shape).astype('float32')) for shape in SHAPES] # update to weights
        ...
        self.weights = WPREV + GPREV + LR + NUM + DEN + WUPDATE + GCURR # lr display hack
        ...
        for i in range(len(params)):
            ...
            self.updates.append(K.update(WUPDATE[i], K.abs(((wcurr-wprev)*(wcurr-wprev)).sum()/(((wcurr-wprev)*(gcurr-gprev)).sum()+1e-8)) * gcurr))
            #### ####

            # updates internal
            self.updates.append(K.update(wprev, wcurr))
            self.updates.append(K.update(gprev, gcurr))
